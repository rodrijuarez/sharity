...
public class ConstantsTei extends TagExtraInfo {
    ...
    public VariableInfo[] getVariableInfo(final TagData data) {
        ...
        try {
            ...
            final Class<?> classInstance = Class.forName(clazz);
            ...
            if (data.getAttributeString("var") == null) {
                ...
                for (final Field field : fields) {
                    ...
                    vars.add(new VariableInfo(field.getName(), ((field.getType().isArray()) ? type.substring(2, type.length() - 1) + "[]" : type), true,
                            VariableInfo.AT_END));
                }
            } else {
                final String var = data.getAttributeString("var");
                final String type = classInstance.getField(var).getType().getName();
                vars.add(new VariableInfo(classInstance.getField(var).getName(),
                        ((classInstance.getField(var).getType().isArray()) ? type.substring(2, type.length() - 1) + "[]" : type), true, VariableInfo.AT_END));
            }
